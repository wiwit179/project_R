...
#import data csv
df <- read.table("C://Users//Wito_Randomo//Desktop//magang//clustering in R//baru//HUAWEI-DAILY-TGL-01-30Sept.csv",header=TRUE,sep=",")
data <- df[,c(2,3,6,8,9,46,47,48)]

#scale kolom oasr, peak traffic, pocc
data1 <- scale(data[,6:8])
View(data1)

#gabungin kolom
data2 <- cbind(data,data1)
data2 <- data2[,c(1,2,3,4,5,9,10,11)]

str(data2)

#pilih kolom oasr, peak traffic, pocc saja
data3 <- data2[,c(6,7,8)]
...
data_final <- data_final[,c(1,2,3,4,5,9,10,11,12)]
...
ggplot(kluster_5, aes(x = Opr)) + stat_count(width = 0.6, aes(fill = sub_opr)) + 
  labs(title = "Berdasarkan sub operator dan opr") 
